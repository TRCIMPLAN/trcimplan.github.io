...
class SociedadMortalidad extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetasnbszlrpl">
...
Fuente Registros Administrativos INEGI, con estimaciones de población de CONAPO.
    ...
    <div class="tab-pane" id="mapa">
              <div id="LeafLetypnduejp" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetasnbszlrpl a:first').tab('show')
});
// LENGUETA
$('#Lenguetasnbszlrpl a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var mapypnduejp;
  ...
  function initmapypnduejp() {
    // Nuevo Mapa
    mapypnduejp = new L.Map('LeafLetypnduejp');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    mapypnduejp.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    mapypnduejp.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmapypnduejp === 'undefined') {
    varinitmapypnduejp = initmapypnduejp();
  };
});
FINAL;
    } // constructor
