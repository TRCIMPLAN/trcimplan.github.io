...
class EconomiaTotalDeEmpresas extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetasevecurqk">
...
<div id="Morrisixcodxov" class="grafica"></div>


    </div>
    <div class="tab-pane" id="mapa">
              <div id="LeafLethmdtprdk" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetasevecurqk a:first').tab('show')
});
// LENGUETA
$('#Lenguetasevecurqk a[href="#grafica"]').on('shown.bs.tab', function (e) {
  // Gráfica
  if (typeof varMorrisixcodxov === 'undefined') {
    varMorrisixcodxov = Morris.Line({
      element: 'Morrisixcodxov',
...
// LENGUETA
$('#Lenguetasevecurqk a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var maphmdtprdk;
  ...
  function initmaphmdtprdk() {
    // Nuevo Mapa
    maphmdtprdk = new L.Map('LeafLethmdtprdk');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    maphmdtprdk.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    maphmdtprdk.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmaphmdtprdk === 'undefined') {
    varinitmaphmdtprdk = initmaphmdtprdk();
  };
});
FINAL;
    } // constructor
