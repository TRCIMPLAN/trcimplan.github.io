...
class SeguridadPorcentajeDeAccidentesVialesFatales extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetashmsllyoj">
...
Porcentaje
    ...
    <div class="tab-pane" id="mapa">
              <div id="LeafLettrabuzqn" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetashmsllyoj a:first').tab('show')
});
// LENGUETA
$('#Lenguetashmsllyoj a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var maptrabuzqn;
  ...
  function initmaptrabuzqn() {
    // Nuevo Mapa
    maptrabuzqn = new L.Map('LeafLettrabuzqn');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    maptrabuzqn.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    maptrabuzqn.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmaptrabuzqn === 'undefined') {
    varinitmaptrabuzqn = initmaptrabuzqn();
  };
});
FINAL;
    } // constructor
