...
class EconomiaIndiceDeGini extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetasmdsltcvp">
...
Datos obtenidos de [CONEVAL](http://www.coneval.gob.mx/Paginas/principal.aspx)
    ...
    <div class="tab-pane" id="mapa">
              <div id="LeafLethbkzlpxd" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetasmdsltcvp a:first').tab('show')
});
// LENGUETA
$('#Lenguetasmdsltcvp a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var maphbkzlpxd;
  ...
  function initmaphbkzlpxd() {
    // Nuevo Mapa
    maphbkzlpxd = new L.Map('LeafLethbkzlpxd');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    maphbkzlpxd.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    maphbkzlpxd.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmaphbkzlpxd === 'undefined') {
    varinitmaphbkzlpxd = initmaphbkzlpxd();
  };
});
FINAL;
    } // constructor
