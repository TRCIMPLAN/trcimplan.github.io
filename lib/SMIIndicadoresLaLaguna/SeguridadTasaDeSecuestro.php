...
class SeguridadTasaDeSecuestro extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetaszezsxorl">
...
Por cada 100 mil
    ...
    <div class="tab-pane" id="mapa">
              <div id="LeafLetzngcezxl" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetaszezsxorl a:first').tab('show')
});
// LENGUETA
$('#Lenguetaszezsxorl a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var mapzngcezxl;
  ...
  function initmapzngcezxl() {
    // Nuevo Mapa
    mapzngcezxl = new L.Map('LeafLetzngcezxl');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    mapzngcezxl.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    mapzngcezxl.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmapzngcezxl === 'undefined') {
    varinitmapzngcezxl = initmapzngcezxl();
  };
});
FINAL;
    } // constructor
