...
class SeguridadPorcentajeDeAccidentesVialesFatales extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetasbeesydci">
...
Porcentaje
    ...
    <div class="tab-pane" id="mapa">
              <div id="LeafLetotadtpqx" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetasbeesydci a:first').tab('show')
});
// LENGUETA
$('#Lenguetasbeesydci a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var mapotadtpqx;
  ...
  function initmapotadtpqx() {
    // Nuevo Mapa
    mapotadtpqx = new L.Map('LeafLetotadtpqx');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    mapotadtpqx.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    mapotadtpqx.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmapotadtpqx === 'undefined') {
    varinitmapotadtpqx = initmapotadtpqx();
  };
});
FINAL;
    } // constructor
