...
class SeguridadPorcentajeDeAccidentesVialesFatales extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetasyclfdsww">
...
Porcentaje
    ...
    <div class="tab-pane" id="mapa">
              <div id="LeafLetkddepdgt" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetasyclfdsww a:first').tab('show')
});
// LENGUETA
$('#Lenguetasyclfdsww a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var mapkddepdgt;
  ...
  function initmapkddepdgt() {
    // Nuevo Mapa
    mapkddepdgt = new L.Map('LeafLetkddepdgt');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    mapkddepdgt.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    mapkddepdgt.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmapkddepdgt === 'undefined') {
    varinitmapkddepdgt = initmapkddepdgt();
  };
});
FINAL;
    } // constructor
