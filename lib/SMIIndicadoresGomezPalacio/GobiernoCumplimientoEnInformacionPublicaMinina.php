...
class GobiernoCumplimientoEnInformacionPublicaMinina extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetasejuatqes">
...
Porcentaje
    ...
    <div class="tab-pane" id="mapa">
              <div id="LeafLetfpblcelx" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetasejuatqes a:first').tab('show')
});
// LENGUETA
$('#Lenguetasejuatqes a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var mapfpblcelx;
  ...
  function initmapfpblcelx() {
    // Nuevo Mapa
    mapfpblcelx = new L.Map('LeafLetfpblcelx');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    mapfpblcelx.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    mapfpblcelx.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmapfpblcelx === 'undefined') {
    varinitmapfpblcelx = initmapfpblcelx();
  };
});
FINAL;
    } // constructor
