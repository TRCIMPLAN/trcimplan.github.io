...
class EconomiaSectoresQueHanFrenadoElCrecimiento20082012 extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetasphxohpfa">
...
Datos obtenidos de [INEGI. Censos económicos](http://www3.inegi.org.mx/sistemas/saic/) y [Centro de Estudios de las Finanzas Públicas](http://www.cefp.gob.mx/Pub_Macro_Estadisticas.htm)
    ...
    <div class="tab-pane" id="mapa">
              <div id="LeafLetgytrbnon" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetasphxohpfa a:first').tab('show')
});
// LENGUETA
$('#Lenguetasphxohpfa a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var mapgytrbnon;
  ...
  function initmapgytrbnon() {
    // Nuevo Mapa
    mapgytrbnon = new L.Map('LeafLetgytrbnon');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    mapgytrbnon.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    mapgytrbnon.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmapgytrbnon === 'undefined') {
    varinitmapgytrbnon = initmapgytrbnon();
  };
});
FINAL;
    } // constructor
