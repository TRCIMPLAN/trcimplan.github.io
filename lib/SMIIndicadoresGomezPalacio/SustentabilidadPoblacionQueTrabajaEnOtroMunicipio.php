...
class SustentabilidadPoblacionQueTrabajaEnOtroMunicipio extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetasxjgpjeua">
...
Consulta la [Base de Datos](http://www.conapo.gob.mx/en/CONAPO/Catalogo_Sistema_Urbano_Nacional_2012)
    ...
    <div class="tab-pane" id="mapa">
              <div id="LeafLetfrhwctyo" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetasxjgpjeua a:first').tab('show')
});
// LENGUETA
$('#Lenguetasxjgpjeua a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var mapfrhwctyo;
  ...
  function initmapfrhwctyo() {
    // Nuevo Mapa
    mapfrhwctyo = new L.Map('LeafLetfrhwctyo');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    mapfrhwctyo.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    mapfrhwctyo.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmapfrhwctyo === 'undefined') {
    varinitmapfrhwctyo = initmapfrhwctyo();
  };
});
FINAL;
    } // constructor
