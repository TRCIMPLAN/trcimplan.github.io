...
class SociedadMortalidadMaterna extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetasxhtryiin">
...
Dirección General de Información en Salud (DGIS). Base de datos de defunciones generales 1979-2007. [en línea]: Sistema Nacional de Información en Salud (SINAIS). [México]: Secretaría de Salud. [Consulta: 01 abril 2014]
    ...
    <div class="tab-pane" id="mapa">
              <div id="LeafLetsrzqbzdm" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetasxhtryiin a:first').tab('show')
});
// LENGUETA
$('#Lenguetasxhtryiin a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var mapsrzqbzdm;
  ...
  function initmapsrzqbzdm() {
    // Nuevo Mapa
    mapsrzqbzdm = new L.Map('LeafLetsrzqbzdm');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    mapsrzqbzdm.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    mapsrzqbzdm.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmapsrzqbzdm === 'undefined') {
    varinitmapsrzqbzdm = initmapsrzqbzdm();
  };
});
FINAL;
    } // constructor
