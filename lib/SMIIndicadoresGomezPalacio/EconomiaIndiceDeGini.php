...
class EconomiaIndiceDeGini extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetaszhvttylj">
...
Datos obtenidos de [CONEVAL](http://www.coneval.gob.mx/Paginas/principal.aspx)
    ...
    <div class="tab-pane" id="mapa">
              <div id="LeafLethpwgonem" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetaszhvttylj a:first').tab('show')
});
// LENGUETA
$('#Lenguetaszhvttylj a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var maphpwgonem;
  ...
  function initmaphpwgonem() {
    // Nuevo Mapa
    maphpwgonem = new L.Map('LeafLethpwgonem');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    maphpwgonem.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    maphpwgonem.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmaphpwgonem === 'undefined') {
    varinitmaphpwgonem = initmaphpwgonem();
  };
});
FINAL;
    } // constructor
