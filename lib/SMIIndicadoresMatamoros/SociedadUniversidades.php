...
class SociedadUniversidades extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetashdkrslmw">
...
* Universidad Pedagógica de Durango-UGP
    ...
    <div class="tab-pane" id="mapa">
              <div id="LeafLethwepmfue" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetashdkrslmw a:first').tab('show')
});
// LENGUETA
$('#Lenguetashdkrslmw a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var maphwepmfue;
  ...
  function initmaphwepmfue() {
    // Nuevo Mapa
    maphwepmfue = new L.Map('LeafLethwepmfue');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    maphwepmfue.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    maphwepmfue.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmaphwepmfue === 'undefined') {
    varinitmaphwepmfue = initmaphwepmfue();
  };
});
FINAL;
    } // constructor
