...
class SustentabilidadRedCarreteraDeAlimentadorasEstatales extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetasnfiecxcx">
...
No aplica a nivel metropolitano
    ...
    <div class="tab-pane" id="mapa">
              <div id="LeafLetyjpkrdge" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetasnfiecxcx a:first').tab('show')
});
// LENGUETA
$('#Lenguetasnfiecxcx a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var mapyjpkrdge;
  ...
  function initmapyjpkrdge() {
    // Nuevo Mapa
    mapyjpkrdge = new L.Map('LeafLetyjpkrdge');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    mapyjpkrdge.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    mapyjpkrdge.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmapyjpkrdge === 'undefined') {
    varinitmapyjpkrdge = initmapyjpkrdge();
  };
});
FINAL;
    } // constructor
