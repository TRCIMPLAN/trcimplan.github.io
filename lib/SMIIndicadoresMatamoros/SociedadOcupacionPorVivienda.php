...
class SociedadOcupacionPorVivienda extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetastwiofnab">
...
Datos obtenidos de [INEGI. Censos de población y vivienda 2010](http://www.inegi.org.mx/sistemas/consulta_resultados/iter2010.aspx)
    ...
    <div class="tab-pane" id="mapa">
              <div id="LeafLethpsshage" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetastwiofnab a:first').tab('show')
});
// LENGUETA
$('#Lenguetastwiofnab a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var maphpsshage;
  ...
  function initmaphpsshage() {
    // Nuevo Mapa
    maphpsshage = new L.Map('LeafLethpsshage');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    maphpsshage.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    maphpsshage.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmaphpsshage === 'undefined') {
    varinitmaphpsshage = initmaphpsshage();
  };
});
FINAL;
    } // constructor
