...
class SeguridadTasaDeRoboDeVehiculos extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetasmibbjiwm">
...
Por cada 100 mil
    ...
    <div class="tab-pane" id="mapa">
              <div id="LeafLetqlnerchw" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetasmibbjiwm a:first').tab('show')
});
// LENGUETA
$('#Lenguetasmibbjiwm a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var mapqlnerchw;
  ...
  function initmapqlnerchw() {
    // Nuevo Mapa
    mapqlnerchw = new L.Map('LeafLetqlnerchw');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    mapqlnerchw.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    mapqlnerchw.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmapqlnerchw === 'undefined') {
    varinitmapqlnerchw = initmapqlnerchw();
  };
});
FINAL;
    } // constructor
