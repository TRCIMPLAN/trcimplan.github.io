...
class EconomiaProductividadLaboral extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetaspzosegmk">
...
Indicador tomado del Índice de Competitividad Urbana 2012 del Instituto Mexicano para la Competitividad. Ir a [base de datos](http://porciudad.comparadondevives.org/contacto)
    ...
    <div class="tab-pane" id="mapa">
              <div id="LeafLetfwftltza" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetaspzosegmk a:first').tab('show')
});
// LENGUETA
$('#Lenguetaspzosegmk a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var mapfwftltza;
  ...
  function initmapfwftltza() {
    // Nuevo Mapa
    mapfwftltza = new L.Map('LeafLetfwftltza');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    mapfwftltza.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    mapfwftltza.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmapfwftltza === 'undefined') {
    varinitmapfwftltza = initmapfwftltza();
  };
});
FINAL;
    } // constructor
