...
class EconomiaUnidadesEconomicasDedicadasALaIndustriaManufacturera extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetasqkoeodcg">
...
<div id="Morrisgmbyyiug" class="grafica"></div>


    </div>
    <div class="tab-pane" id="mapa">
              <div id="LeafLethbnkdsuj" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetasqkoeodcg a:first').tab('show')
});
// LENGUETA
$('#Lenguetasqkoeodcg a[href="#grafica"]').on('shown.bs.tab', function (e) {
  // Gráfica
  if (typeof varMorrisgmbyyiug === 'undefined') {
    varMorrisgmbyyiug = Morris.Line({
      element: 'Morrisgmbyyiug',
...
// LENGUETA
$('#Lenguetasqkoeodcg a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var maphbnkdsuj;
  ...
  function initmaphbnkdsuj() {
    // Nuevo Mapa
    maphbnkdsuj = new L.Map('LeafLethbnkdsuj');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    maphbnkdsuj.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    maphbnkdsuj.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmaphbnkdsuj === 'undefined') {
    varinitmaphbnkdsuj = initmaphbnkdsuj();
  };
});
FINAL;
    } // constructor
