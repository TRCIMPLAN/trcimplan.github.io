...
class SeguridadTasaDeDelitos extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetassnipsojy">
...
Cantidad
    ...
    <div class="tab-pane" id="mapa">
              <div id="LeafLetcpdrdtpq" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetassnipsojy a:first').tab('show')
});
// LENGUETA
$('#Lenguetassnipsojy a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var mapcpdrdtpq;
  ...
  function initmapcpdrdtpq() {
    // Nuevo Mapa
    mapcpdrdtpq = new L.Map('LeafLetcpdrdtpq');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    mapcpdrdtpq.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    mapcpdrdtpq.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmapcpdrdtpq === 'undefined') {
    varinitmapcpdrdtpq = initmapcpdrdtpq();
  };
});
FINAL;
    } // constructor
