...
class SustentabilidadAccesoAVialidadesPavimentadas extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetasbxnuvffw">
...
Censo de Población y Vivienda 2010. Consulta las [Bases de Datos](http://www.inegi.org.mx/est/contenidos/proyectos/ccpv/cpv2010/tabulados_urbano.aspx)
    ...
    <div class="tab-pane" id="mapa">
              <div id="LeafLetxujkbysi" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetasbxnuvffw a:first').tab('show')
});
// LENGUETA
$('#Lenguetasbxnuvffw a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var mapxujkbysi;
  ...
  function initmapxujkbysi() {
    // Nuevo Mapa
    mapxujkbysi = new L.Map('LeafLetxujkbysi');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    mapxujkbysi.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    mapxujkbysi.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmapxujkbysi === 'undefined') {
    varinitmapxujkbysi = initmapxujkbysi();
  };
});
FINAL;
    } // constructor
