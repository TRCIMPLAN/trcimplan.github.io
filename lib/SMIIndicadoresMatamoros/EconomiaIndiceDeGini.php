...
class EconomiaIndiceDeGini extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetasvovhzmui">
...
Datos obtenidos de [CONEVAL](http://www.coneval.gob.mx/Paginas/principal.aspx)
    ...
    <div class="tab-pane" id="mapa">
              <div id="LeafLetknfuujrm" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetasvovhzmui a:first').tab('show')
});
// LENGUETA
$('#Lenguetasvovhzmui a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var mapknfuujrm;
  ...
  function initmapknfuujrm() {
    // Nuevo Mapa
    mapknfuujrm = new L.Map('LeafLetknfuujrm');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    mapknfuujrm.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    mapknfuujrm.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmapknfuujrm === 'undefined') {
    varinitmapknfuujrm = initmapknfuujrm();
  };
});
FINAL;
    } // constructor
