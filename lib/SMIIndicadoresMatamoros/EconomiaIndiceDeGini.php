...
class EconomiaIndiceDeGini extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetasfdgkanjo">
...
Datos obtenidos de [CONEVAL](http://www.coneval.gob.mx/Paginas/principal.aspx)
    ...
    <div class="tab-pane" id="mapa">
              <div id="LeafLetkzldccnd" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetasfdgkanjo a:first').tab('show')
});
// LENGUETA
$('#Lenguetasfdgkanjo a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var mapkzldccnd;
  ...
  function initmapkzldccnd() {
    // Nuevo Mapa
    mapkzldccnd = new L.Map('LeafLetkzldccnd');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    mapkzldccnd.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    mapkzldccnd.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmapkzldccnd === 'undefined') {
    varinitmapkzldccnd = initmapkzldccnd();
  };
});
FINAL;
    } // constructor
