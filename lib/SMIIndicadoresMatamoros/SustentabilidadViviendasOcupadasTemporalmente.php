...
class SustentabilidadViviendasOcupadasTemporalmente extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetascspyyzvd">
...
Porcentaje
    ...
    <div class="tab-pane" id="mapa">
              <div id="LeafLethqdkxgmq" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetascspyyzvd a:first').tab('show')
});
// LENGUETA
$('#Lenguetascspyyzvd a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var maphqdkxgmq;
  ...
  function initmaphqdkxgmq() {
    // Nuevo Mapa
    maphqdkxgmq = new L.Map('LeafLethqdkxgmq');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    maphqdkxgmq.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    maphqdkxgmq.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmaphqdkxgmq === 'undefined') {
    varinitmaphqdkxgmq = initmaphqdkxgmq();
  };
});
FINAL;
    } // constructor
