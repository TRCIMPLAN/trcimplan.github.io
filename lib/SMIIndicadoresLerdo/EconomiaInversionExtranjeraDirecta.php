...
class EconomiaInversionExtranjeraDirecta extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetasgxjtqnnj">
...
y [Secretaría de Economía](http://www.economia.gob.mx/comunidad-negocios/competitividad-normatividad/inversion-extranjera-directa/estadistica-oficial-de-ied-en-mexico)
    ...
    <div class="tab-pane" id="mapa">
              <div id="LeafLetljfaredg" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetasgxjtqnnj a:first').tab('show')
});
// LENGUETA
$('#Lenguetasgxjtqnnj a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var mapljfaredg;
  ...
  function initmapljfaredg() {
    // Nuevo Mapa
    mapljfaredg = new L.Map('LeafLetljfaredg');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    mapljfaredg.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    mapljfaredg.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmapljfaredg === 'undefined') {
    varinitmapljfaredg = initmapljfaredg();
  };
});
FINAL;
    } // constructor
