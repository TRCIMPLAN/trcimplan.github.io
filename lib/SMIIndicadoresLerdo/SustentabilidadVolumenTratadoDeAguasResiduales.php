...
class SustentabilidadVolumenTratadoDeAguasResiduales extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetasgunjekgm">
...
Litros por segundo por cada mil habitantes
    ...
    <div class="tab-pane" id="mapa">
              <div id="LeafLetlznvcgki" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetasgunjekgm a:first').tab('show')
});
// LENGUETA
$('#Lenguetasgunjekgm a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var maplznvcgki;
  ...
  function initmaplznvcgki() {
    // Nuevo Mapa
    maplznvcgki = new L.Map('LeafLetlznvcgki');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    maplznvcgki.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    maplznvcgki.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmaplznvcgki === 'undefined') {
    varinitmaplznvcgki = initmaplznvcgki();
  };
});
FINAL;
    } // constructor
