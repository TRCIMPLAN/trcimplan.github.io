...
class SociedadMortalidadInfantil extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetasnvjbhqdp">
...
Por cada mil
    ...
    <div class="tab-pane" id="mapa">
              <div id="LeafLethpieetun" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetasnvjbhqdp a:first').tab('show')
});
// LENGUETA
$('#Lenguetasnvjbhqdp a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var maphpieetun;
  ...
  function initmaphpieetun() {
    // Nuevo Mapa
    maphpieetun = new L.Map('LeafLethpieetun');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    maphpieetun.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    maphpieetun.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmaphpieetun === 'undefined') {
    varinitmaphpieetun = initmaphpieetun();
  };
});
FINAL;
    } // constructor
