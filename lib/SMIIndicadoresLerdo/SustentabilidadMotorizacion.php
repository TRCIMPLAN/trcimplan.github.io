...
class SustentabilidadMotorizacion extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetasdpuhueoc">
...
Índice de motorización. Consulta la [Base de Datos](http://www.inegi.org.mx/sistemas/olap/Proyectos/bd/continuas/transporte/vehiculos.asp?s=est&c=13158&proy=vmrc_vehiculos)
    ...
    <div class="tab-pane" id="mapa">
              <div id="LeafLetawpchggo" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetasdpuhueoc a:first').tab('show')
});
// LENGUETA
$('#Lenguetasdpuhueoc a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var mapawpchggo;
  ...
  function initmapawpchggo() {
    // Nuevo Mapa
    mapawpchggo = new L.Map('LeafLetawpchggo');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    mapawpchggo.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    mapawpchggo.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmapawpchggo === 'undefined') {
    varinitmapawpchggo = initmapawpchggo();
  };
});
FINAL;
    } // constructor
