...
class SociedadPobrezaModerada extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetasuvghgfji">
...
Consulta la [Base de Datos](http://www.coneval.gob.mx/Medicion/Paginas/Medici%C3%B3n/Pobreza%202012/Pobreza-2012.aspx)
    ...
    <div class="tab-pane" id="mapa">
              <div id="LeafLethbypmpux" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetasuvghgfji a:first').tab('show')
});
// LENGUETA
$('#Lenguetasuvghgfji a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var maphbypmpux;
  ...
  function initmaphbypmpux() {
    // Nuevo Mapa
    maphbypmpux = new L.Map('LeafLethbypmpux');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    maphbypmpux.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    maphbypmpux.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmaphbypmpux === 'undefined') {
    varinitmaphbypmpux = initmaphbypmpux();
  };
});
FINAL;
    } // constructor
