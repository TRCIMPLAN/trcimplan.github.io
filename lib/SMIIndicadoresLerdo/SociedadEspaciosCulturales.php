...
class SociedadEspaciosCulturales extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetasaudbhdeq">
...
Se realizó un filtrado básico de la totalidad de los espacios culturales considerados por CONACULTA, ya que ellos consideran programas, fondos y proyectos a sus estadísticas totales, así como eventos que fueron realizados en una ocasión y datan de hace más de 5 años. Consulta la [Base de Datos completa](http://www.sic.gob.mx)
    ...
    <div class="tab-pane" id="mapa">
              <div id="LeafLetsssuxkke" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetasaudbhdeq a:first').tab('show')
});
// LENGUETA
$('#Lenguetasaudbhdeq a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var mapsssuxkke;
  ...
  function initmapsssuxkke() {
    // Nuevo Mapa
    mapsssuxkke = new L.Map('LeafLetsssuxkke');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    mapsssuxkke.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    mapsssuxkke.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmapsssuxkke === 'undefined') {
    varinitmapsssuxkke = initmapsssuxkke();
  };
});
FINAL;
    } // constructor
