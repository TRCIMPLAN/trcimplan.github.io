...
class SustentabilidadDestinosViaAerea extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetasqqlaipfv">
...
Se considera a nivel metropolitano. Consulta la [Base de Datos](http://www.oma.aero/es/aeropuertos/trfico-de-pasajeros/)
    ...
    <div class="tab-pane" id="mapa">
              <div id="LeafLetbxuyqsvm" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetasqqlaipfv a:first').tab('show')
});
// LENGUETA
$('#Lenguetasqqlaipfv a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var mapbxuyqsvm;
  ...
  function initmapbxuyqsvm() {
    // Nuevo Mapa
    mapbxuyqsvm = new L.Map('LeafLetbxuyqsvm');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    mapbxuyqsvm.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    mapbxuyqsvm.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmapbxuyqsvm === 'undefined') {
    varinitmapbxuyqsvm = initmapbxuyqsvm();
  };
});
FINAL;
    } // constructor
