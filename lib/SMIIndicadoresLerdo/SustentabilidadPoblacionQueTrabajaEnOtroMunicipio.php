...
class SustentabilidadPoblacionQueTrabajaEnOtroMunicipio extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetasuqowsclt">
...
Consulta la [Base de Datos](http://www.conapo.gob.mx/en/CONAPO/Catalogo_Sistema_Urbano_Nacional_2012)
    ...
    <div class="tab-pane" id="mapa">
              <div id="LeafLetkdetlhnr" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetasuqowsclt a:first').tab('show')
});
// LENGUETA
$('#Lenguetasuqowsclt a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var mapkdetlhnr;
  ...
  function initmapkdetlhnr() {
    // Nuevo Mapa
    mapkdetlhnr = new L.Map('LeafLetkdetlhnr');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    mapkdetlhnr.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    mapkdetlhnr.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmapkdetlhnr === 'undefined') {
    varinitmapkdetlhnr = initmapkdetlhnr();
  };
});
FINAL;
    } // constructor
