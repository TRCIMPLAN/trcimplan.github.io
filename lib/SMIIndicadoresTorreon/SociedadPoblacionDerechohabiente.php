...
class SociedadPoblacionDerechohabiente extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetasdljkxxap">
...
Fuente: INEGI
    ...
    <div class="tab-pane" id="mapa">
              <div id="LeafLetryhydrgw" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetasdljkxxap a:first').tab('show')
});
// LENGUETA
$('#Lenguetasdljkxxap a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var mapryhydrgw;
  ...
  function initmapryhydrgw() {
    // Nuevo Mapa
    mapryhydrgw = new L.Map('LeafLetryhydrgw');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    mapryhydrgw.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    mapryhydrgw.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmapryhydrgw === 'undefined') {
    varinitmapryhydrgw = initmapryhydrgw();
  };
});
FINAL;
    } // constructor
