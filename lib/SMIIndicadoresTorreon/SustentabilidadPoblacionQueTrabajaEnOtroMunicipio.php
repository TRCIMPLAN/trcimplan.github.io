...
class SustentabilidadPoblacionQueTrabajaEnOtroMunicipio extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetaslggnunfb">
...
Consulta la [Base de Datos](http://www.conapo.gob.mx/en/CONAPO/Catalogo_Sistema_Urbano_Nacional_2012)
    ...
    <div class="tab-pane" id="mapa">
              <div id="LeafLetfjmsixcu" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetaslggnunfb a:first').tab('show')
});
// LENGUETA
$('#Lenguetaslggnunfb a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var mapfjmsixcu;
  ...
  function initmapfjmsixcu() {
    // Nuevo Mapa
    mapfjmsixcu = new L.Map('LeafLetfjmsixcu');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    mapfjmsixcu.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    mapfjmsixcu.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmapfjmsixcu === 'undefined') {
    varinitmapfjmsixcu = initmapfjmsixcu();
  };
});
FINAL;
    } // constructor
