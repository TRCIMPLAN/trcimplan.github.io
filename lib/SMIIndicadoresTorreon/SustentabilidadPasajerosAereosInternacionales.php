...
class SustentabilidadPasajerosAereosInternacionales extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetassnsucigz">
...
<div id="Morriscgldfvru" class="grafica"></div>


    </div>
    <div class="tab-pane" id="mapa">
              <div id="LeafLettctdfoaa" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetassnsucigz a:first').tab('show')
});
// LENGUETA
$('#Lenguetassnsucigz a[href="#grafica"]').on('shown.bs.tab', function (e) {
  // Gráfica
  if (typeof varMorriscgldfvru === 'undefined') {
    varMorriscgldfvru = Morris.Line({
      element: 'Morriscgldfvru',
...
// LENGUETA
$('#Lenguetassnsucigz a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var maptctdfoaa;
  ...
  function initmaptctdfoaa() {
    // Nuevo Mapa
    maptctdfoaa = new L.Map('LeafLettctdfoaa');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    maptctdfoaa.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    maptctdfoaa.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmaptctdfoaa === 'undefined') {
    varinitmaptctdfoaa = initmaptctdfoaa();
  };
});
FINAL;
    } // constructor
