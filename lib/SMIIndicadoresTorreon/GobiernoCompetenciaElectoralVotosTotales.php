...
class GobiernoCompetenciaElectoralVotosTotales extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetasovfgujqb">
...
Cantidad
    ...
    <div class="tab-pane" id="mapa">
              <div id="LeafLetgenugscm" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetasovfgujqb a:first').tab('show')
});
// LENGUETA
$('#Lenguetasovfgujqb a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var mapgenugscm;
  ...
  function initmapgenugscm() {
    // Nuevo Mapa
    mapgenugscm = new L.Map('LeafLetgenugscm');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    mapgenugscm.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    mapgenugscm.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmapgenugscm === 'undefined') {
    varinitmapgenugscm = initmapgenugscm();
  };
});
FINAL;
    } // constructor
