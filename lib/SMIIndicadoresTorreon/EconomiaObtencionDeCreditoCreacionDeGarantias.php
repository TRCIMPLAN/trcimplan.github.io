...
class EconomiaObtencionDeCreditoCreacionDeGarantias extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetasfgxkyszj">
...
Días
    ...
    <div class="tab-pane active" id="mapa">
              <div id="LeafLetothodpzm" class="mapa"></div>
    </div>
  </div>
FINAL;
...
$(document).ready(function(){
  $('#Lenguetasfgxkyszj a:first').tab('show')
});
// LENGUETA
$('#Lenguetasfgxkyszj a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var mapothodpzm;
  ...
  function initmapothodpzm() {
    // Nuevo Mapa
    mapothodpzm = new L.Map('LeafLetothodpzm');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    mapothodpzm.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    mapothodpzm.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmapothodpzm === 'undefined') {
    varinitmapothodpzm = initmapothodpzm();
  };
});
FINAL;
    } // constructor
