...
class EconomiaPequenasEmpresas extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetasaxqouvgy">
...
<div id="Morrispcswtqda" class="grafica"></div>


    </div>
    <div class="tab-pane" id="mapa">
              <div id="LeafLetkvgyxyql" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetasaxqouvgy a:first').tab('show')
});
// LENGUETA
$('#Lenguetasaxqouvgy a[href="#grafica"]').on('shown.bs.tab', function (e) {
  // Gráfica
  if (typeof varMorrispcswtqda === 'undefined') {
    varMorrispcswtqda = Morris.Line({
      element: 'Morrispcswtqda',
...
// LENGUETA
$('#Lenguetasaxqouvgy a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var mapkvgyxyql;
  ...
  function initmapkvgyxyql() {
    // Nuevo Mapa
    mapkvgyxyql = new L.Map('LeafLetkvgyxyql');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    mapkvgyxyql.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    mapkvgyxyql.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmapkvgyxyql === 'undefined') {
    varinitmapkvgyxyql = initmapkvgyxyql();
  };
});
FINAL;
    } // constructor
