...
class SociedadViviendasConEnergiaElectrica extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetasqtrnnrsu">
...
Enlace al [Sistema de Información Geográfica](http://201.159.104.45:8080/apps/implan2.html).
    ...
    <div class="tab-pane" id="mapa">
              <div id="LeafLettyopcbsi" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetasqtrnnrsu a:first').tab('show')
});
// LENGUETA
$('#Lenguetasqtrnnrsu a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var maptyopcbsi;
  ...
  function initmaptyopcbsi() {
    // Nuevo Mapa
    maptyopcbsi = new L.Map('LeafLettyopcbsi');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    maptyopcbsi.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    maptyopcbsi.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmaptyopcbsi === 'undefined') {
    varinitmaptyopcbsi = initmaptyopcbsi();
  };
});
FINAL;
    } // constructor
