...
class GobiernoCompetenciaElectoralDiferenciaDeVotos extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetaskefvwzma">
...
Cantidad de votos
    ...
    <div class="tab-pane" id="mapa">
              <div id="LeafLetgsnmuzms" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetaskefvwzma a:first').tab('show')
});
// LENGUETA
$('#Lenguetaskefvwzma a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var mapgsnmuzms;
  ...
  function initmapgsnmuzms() {
    // Nuevo Mapa
    mapgsnmuzms = new L.Map('LeafLetgsnmuzms');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    mapgsnmuzms.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    mapgsnmuzms.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmapgsnmuzms === 'undefined') {
    varinitmapgsnmuzms = initmapgsnmuzms();
  };
});
FINAL;
    } // constructor
