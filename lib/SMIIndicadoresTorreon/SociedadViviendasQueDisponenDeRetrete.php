...
class SociedadViviendasQueDisponenDeRetrete extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetasklwepoic">
...
Enlace al [Sistema de Información Geográfica](http://201.159.104.45:8080/apps/implan2.html).
    ...
    <div class="tab-pane" id="mapa">
              <div id="LeafLetepsspbny" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetasklwepoic a:first').tab('show')
});
// LENGUETA
$('#Lenguetasklwepoic a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var mapepsspbny;
  ...
  function initmapepsspbny() {
    // Nuevo Mapa
    mapepsspbny = new L.Map('LeafLetepsspbny');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    mapepsspbny.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    mapepsspbny.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmapepsspbny === 'undefined') {
    varinitmapepsspbny = initmapepsspbny();
  };
});
FINAL;
    } // constructor
