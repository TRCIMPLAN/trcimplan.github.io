...
class SeguridadTasaDeHomicidios extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetaskbnosugu">
...
<b>Unidad:</b> Por cada 100 mil.
    ...
    <div class="tab-pane" id="mapa">
              <div id="LeafLetluncpsfv" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetaskbnosugu a:first').tab('show')
});
// LENGUETA
$('#Lenguetaskbnosugu a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var mapluncpsfv;
  ...
  function initmapluncpsfv() {
    // Nuevo Mapa
    mapluncpsfv = new L.Map('LeafLetluncpsfv');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    mapluncpsfv.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    mapluncpsfv.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmapluncpsfv === 'undefined') {
    varinitmapluncpsfv = initmapluncpsfv();
  };
});
FINAL;
    } // constructor
