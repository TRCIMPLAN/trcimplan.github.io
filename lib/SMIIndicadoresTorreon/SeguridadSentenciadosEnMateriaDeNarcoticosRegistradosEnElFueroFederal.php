...
class SeguridadSentenciadosEnMateriaDeNarcoticosRegistradosEnElFueroFederal extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetasbkkvkfwp">
...
Cantidad
    ...
    <div class="tab-pane" id="mapa">
              <div id="LeafLethbekstdz" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetasbkkvkfwp a:first').tab('show')
});
// LENGUETA
$('#Lenguetasbkkvkfwp a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var maphbekstdz;
  ...
  function initmaphbekstdz() {
    // Nuevo Mapa
    maphbekstdz = new L.Map('LeafLethbekstdz');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    maphbekstdz.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    maphbekstdz.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmaphbekstdz === 'undefined') {
    varinitmaphbekstdz = initmaphbekstdz();
  };
});
FINAL;
    } // constructor
