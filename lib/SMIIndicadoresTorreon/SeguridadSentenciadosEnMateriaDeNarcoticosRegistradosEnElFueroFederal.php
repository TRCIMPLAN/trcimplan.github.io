...
class SeguridadSentenciadosEnMateriaDeNarcoticosRegistradosEnElFueroFederal extends \Base\Publicacion {
  ...
  <ul class="nav nav-tabs lenguetas" id="Lenguetasnmubetho">
...
Cantidad
    ...
    <div class="tab-pane" id="mapa">
              <div id="LeafLetowvnslpc" class="mapa"></div>
    </div>
...
$(document).ready(function(){
  $('#Lenguetasnmubetho a:first').tab('show')
});
// LENGUETA
$('#Lenguetasnmubetho a[href="#mapa"]').on('shown.bs.tab', function (e) {
  // Mapa
  var mapowvnslpc;
  ...
  function initmapowvnslpc() {
    // Nuevo Mapa
    mapowvnslpc = new L.Map('LeafLetowvnslpc');
    // Capa con el mapa
    var osmUrl='http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
    var osmAttrib='Ayuntamiento de Torreón. Map data © OpenStreetMap contributors';
    var osm = new L.TileLayer(osmUrl, {minZoom: 12, maxZoom: 18, attribution: osmAttrib});
    // Definir coordenadas del centro del mapa y el nivel de zoom
    mapowvnslpc.setView(new L.LatLng(25.54, -103.44), 12);
    // Agregar capa con el mapa
    mapowvnslpc.addLayer(osm);
  ...
  // Ejecutar el mapa
  if (typeof varinitmapowvnslpc === 'undefined') {
    varinitmapowvnslpc = initmapowvnslpc();
  };
});
FINAL;
    } // constructor
